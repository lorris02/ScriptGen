"""
Script Generator Bot — Bot 4 (Gemini Version)
Features:
...
Target length: 8-12 minutes when read aloud (roughly 1200-1800 words).
...
def build_doc_prompt(topic, style, niche):
    return f"""You are an expert documentary YouTube scriptwriter. Cinematic, narrative-driven, deeply researched.

Write a full documentary-style YouTube script about: "{topic}"

Target: 12-15 minutes (1800-2200 words). Niche: {niche}. Tone: {style}.
...
COLD OPEN (cinematic hook — intrigue before context):
[script]
...
OUTRO + CTA:
[reflective close + subscribe CTA + comment question]
...
A creator shared this transcript from a video they liked:

---TRANSCRIPT START---
{transcript}
---TRANSCRIPT END---

Analyze the structure and storytelling approach, then write a COMPLETELY ORIGINAL script inspired by it. NOT a copy.

Requirements:
- Format: {format_map.get(fmt, "YouTube Shorts")}
- Style: {style.upper()}
- Niche: {niche}
- Same energy and structure that made the original effective
- 100% original topics, examples, and content

Format EXACTLY like this:

WHAT MADE THE ORIGINAL WORK:
[2-3 sentences on structure/style detected]
...
Generate 5 completely different viral angle approaches for this topic.
...
Make each angle genuinely different — vary emotion, framing, and target audience."""

def build_rewrite_prompt(script, style, niche):
    return f"""You are an expert scriptwriter. Rewrite and dramatically improve this script.
...
WHAT YOU CHANGED:
[3 bullet points on key improvements]"""
...
async def cmd_shortscript(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
    ...
    try:
        result = ask_gemini(build_short_prompt(topic, prefs["style"], prefs["niche"]))
        for i in range(0, len(result), 4000):
            await update.message.reply_text(result[i:i+4000])
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {e}")

async def cmd_longscript(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
    ...
    try:
        result = ask_gemini(build_long_prompt(topic, prefs["style"], prefs["niche"]))
        for i in range(0, len(result), 4000):
            await update.message.reply_text(result[i:i+4000])
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {e}")

async def cmd_docscript(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
    ...
    try:
        result = ask_gemini(build_doc_prompt(topic, prefs["style"], prefs["niche"]))
        for i in range(0, len(result), 4000):
            await update.message.reply_text(result[i:i+4000])
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {e}")

async def cmd_replicate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    prefs = get_prefs(update.effective_user.id)
    set_awaiting_replicate(update.effective_user.id, 1)
    await update.message.reply_text(
        f"📋 *Replicate Mode ON*\n\n"
        f"Paste the transcript you want to replicate.\n\n"
        f"Optionally add at the end:\n"
        f"`[style: funny] [format: short]`\n\n"
        f"Available styles: {', '.join(STYLES)}\n"
        f"Available formats: {', '.join(FORMATS)}\n\n"
        f"Your defaults — style: *{prefs['style']}*, format: *short*",
        parse_mode="Markdown"
    )
...
async def cmd_rewrite(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        result = ask_gemini(build_rewrite_prompt(script, prefs["style"], prefs["niche"]))
        for i in range(0, len(result), 4000):
            await update.message.reply_text(result[i:i+4000])
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {e}")

async def cmd_angles(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
    ...
    try:
        result = ask_gemini(build_angles_prompt(topic, prefs["niche"]))
        for i in range(0, len(result), 4000):
            await update.message.reply_text(result[i:i+4000])
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {e}")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if prefs["awaiting_replicate"]:
        ...
        await update.message.reply_text(
            f"🔄 Replicating in *{style}* style as *{fmt}* format...",
            parse_mode="Markdown"
        )
        try:
            result = ask_gemini(build_replicate_prompt(transcript, style, fmt, prefs["niche"]))
            for i in range(0, len(result), 4000):
                await update.message.reply_text(result[i:i+4000])
        except Exception as e:
            await update.message.reply_text(f"❌ Error: {e}")
    else:
        await update.message.reply_text("Use a command to get started. Type /help to see all commands.")
...
def main():
    ...
    app.add_handler(CommandHandler("help", cmd_help := cmd_start))
